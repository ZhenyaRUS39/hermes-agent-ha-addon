# Hermes Agent — Аддон для Home Assistant v0.11.0
...
Образы обновляются автоматически при новых релизах на GitHub. Для установки свежей версии:

1. Обновите репозиторий аддона в Home Assistant
2. Переустановите аддон

Теги образов:
- `ghcr.io/zhenyarus39/hermes-agent:v0.11.0` — текущая версия
- `ghcr.io/zhenyarus39/hermes-agent:latest` — последний стабильный

### Вариант B — Через update_on_start
...
При каждом запуске аддон выполнит `git pull` и установит latest из master.

## Порт и доступ

- **Внутренний порт:** `8000`
- **Ingress URL:** автоматически через HA sidebar
- **Host network:** включён — Hermes имеет прямой доступ к сети

## Структура хранилища

```
/data/                              # Persistent share
...
- [Релизы аддона](https://github.com/ZhenyaRUS39/hermes-agent-ha-addon/releases)
